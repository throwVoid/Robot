...
SocketHandler::~SocketHandler()
{
	...
	}
	{
		...
		{
			DEB(fprintf(stderr, "handler析构时正在清空套接字map, 共%d 个实例\n", (int)m_sockets.size());)
			...
			if (p)
			{
				...
				p -> Close();
				DEB(fprintf(stderr, " 描述符关闭-> %d\n", p -> GetSocket());)
//				p -> OnDelete();

				// 仅仅删除被控制的socket
				// non-detached socket能被master socketHandler删除
				// detach socket只能被slave socketHandler删除
				if (p -> DeleteByHandler()
#ifdef ENABLE_DETACH
					//handler运行在SocketThread上，套接字已经分离
					//handler运行没运行在SocketThread上，套接字未分离
			...
			DEB(fprintf(stderr, "继续清空...\n");)
		}
		DEB(fprintf(stderr, "已经被清空, %d 个实列\n", (int)m_sockets.size());)
	}
#ifdef ENABLE_RESOLVER
	//已经指定了解析服务器ResolvServer
	if (m_resolver)
	{
		m_resolver -> Quit();
		delete m_resolver;
...
ISocketHandler& SocketHandler::ParentHandler()
{
	if (!m_b_parent_is_valid)
		throw Exception("没有sockethandler可用");
	return m_parent;
}

//在SocketHandle管理的多线程中找到一个当前事件数最少的SocketHandler
ISocketHandler& SocketHandler::GetRandomHandler()
{
	if (m_threads.empty())
		throw Exception("SocketHandler不是多线程");
...
//如果父handler有效则返回父亲handler，否则返回当前handler
...
//设置线程数，让SockeetHandler在单独的线程中运行
//创建线程的SocketHandler为Parent handler
void SocketHandler::SetNumberOfThreads(size_t n)
{
	...
	if (n > 1 && n < 256)
	{
		for (int i = 1; i <= (int)n; i++)
		{
			...
			/*
			 *在创建SocketHandlerThread后，Parent Handler会使用Creat来创建子handler来管理
			 * */
...
int SocketHandler::Resolve(Socket *p,const std::string& host,port_t port)
{
	...
	{
		LogError(resolv, "Resolve", -1, "不能连接到本地DNS解析服务器", LOG_LEVEL_FATAL);
	}
	Add(resolv);
...
void SocketHandler::Remove(Socket *p)
{
...
#endif
	//如果该Socket交给Handler删除，则返回
	if (p -> ErasedByHandler())
	{
		return;
	}
	//检查活跃的socket map中是否该Socket正在被使用
	for (socket_m::iterator it = m_sockets.begin(); it != m_sockets.end(); ++it)
	{
		if (it -> second == p)
		{
			LogError(p, "Remove", -1, "试图析构正在被使用的Socket", LOG_LEVEL_WARNING);
			//从活跃的map中删除
			m_sockets.erase(it);
			return;
		}
	...
	for (std::list<Socket *>::iterator it2 = m_add.begin(); it2 != m_add.end(); ++it2)
	{
		if (*it2 == p)
		{
			LogError(p, "Remove", -2, "试图析构正在被使用的Socket", LOG_LEVEL_WARNING);
			m_add.erase(it2);
			return;
		}
	}
	for (std::list<Socket *>::iterator it3 = m_delete.begin(); it3 != m_delete.end(); ++it3)
	{
		if (*it3 == p)
		{
			LogError(p, "Remove", -3, "试图析构正在被使用的Socket", LOG_LEVEL_WARNING);
			m_delete.erase(it3);
			return;
		}
	}
...
void SocketHandler::RebuildFdset()
{
	...
	for (socket_m::iterator it = m_sockets.begin(); it != m_sockets.end(); ++it)
	{
		...
		if (s == p -> GetSocket() && s >= 0)
		{
			...
			if (n == -1 && Errno == EBADF)
			{
				LogError(p, "Select", (int)s, "无效的fd fd_set (2)", LOG_LEVEL_ERROR);
		...
		{
			LogError(p, "Select", (int)s, "无效的fd fd_set (3)", LOG_LEVEL_ERROR);
			DeleteSocket(p);
		}
	}
	m_rfds = rfds;
...
//将m_add（Add添加的Socket）里的Socket增加到m_sockets里
void SocketHandler::AddIncoming()
{
	while (m_add.size() > 0)
	{
		...
		{
			LogError(NULL, "Select", (int)m_sockets.size(), "超出了Socket限制数量", LOG_LEVEL_WARNING);
			break;
		}
		std::list<Socket *>::iterator it = m_add.begin();
		Socket *p = *it;
		SOCKET s = p -> GetSocket();
		DEB(fprintf(stderr, "尝试增加fd %d,  m_add.size() %d\n", (int)s, (int)m_add.size());)
		//
		if (s == INVALID_SOCKET)
		{
			LogError(p, "Add", -1, "无效的Socket", LOG_LEVEL_WARNING);
			//添加到删除链表中
		...
		if (p -> CloseAndDelete())
		{
			LogError(p, "Add", (int)p -> GetSocket(), "试图增加SetCloseAndDelete()为true的Socket", LOG_LEVEL_WARNING);
			m_sockets[s] = p;
			DeleteSocket(p);
			p -> Close();
		}
...
printf("]\n");
	...
	if (n == -1) // error on select
	{
		int err = Errno;
		/*
			EBADF 给fdset了无效的文件描述符
			EINTR  一个非阻塞信号被捕获
			EINVAL 描述符是负数或则timewal的值小于0
			ENOMEM select不能分配内存
		*/
		switch (err)
		{
		...
		case EINVAL:
			...
			throw Exception("select(n): "描述符是负数或则timewal的值小于0"");
		case ENOMEM:
