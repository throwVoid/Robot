...
RobotSocket::RobotSocket(int port, bool doClose, NetType type) 
:myType(type)
,myError(NoErr)
,myErrorStr()
,myDoClose(doClose)
 ...
 *@doClose 当套接字析构时，是否自动关闭该连接套接字
*/
RobotSocket::RobotSocket(const char* host, int port, bool doClose, NetType type) :   
  myType(type),
  myError(NoErr),
  myErrorStr(),
  myDoClose(doClose),
...
/*
 *  下面的代码是为了配合手柄，这是自己使用手柄时写的例子
 *
 * */
/*下面这个函数是为了配合手柄操作，防止网络建立失败导致一直阻塞在手柄模式*/
/*如果在构造函数中就open，则检测不到失败，导致套接字失败一直没有消息而切换不到智能模式*/
RobotSocket::RobotSocket(/*const char* host, int port,*/ bool doClose, NetType type) :   
  myType(type),
  myError(NoErr),
  myErrorStr(),
 ...
 * 完成套接字，绑定
 * */
bool RobotSocket::open(int port, NetType type, const char *openOnIP)
{
	int ret;
...
bool RobotSocket::findValidPort(int startPort, const char *openOnIP)
{
	...
	//int i = startPort;
	{
		...
		mySin.sin_family=AF_INET;
		mySin.sin_port=hostToNetOrder(startPort);
...
bool RobotSocket::Listen()
{
	return listen(myFD, 5);
}
#ifdef ROBOT_NAMESPACE
}
#endif
